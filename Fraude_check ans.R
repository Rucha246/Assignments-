...
type <- ifelse(Fraud_check$Taxable.Income <= 30000,"Risky","Good")
Fraud_df <- data.frame(Fraud_check,type)
...
# Here we can conclude that in our model that the model is unrelaible untill or unless we get a relaivent variable in our data

summary(lm(Taxable.Income~.,data =df ))$coe
#  looking at the linear model summary between all the variable for predicting the Taxable Income, we cant find any of them significant.
# we are building the linear model to check whether the origin of our desired categorical variable having any variable explanable for its variation or not.
