...
head(wine)
table(wine$Type)
write.csv(summary(wine),"summary.csv")
...
df_norm <- data.frame(Type=wine$Type,normalize_dummy(wine[,-1]))
head(df_norm)
...
clustlist_O <-  hclust(dist = dist_O,k = 3)
...
mean(clusto1==df_norm$Type) # 0.9494382 #0.5674
...
mean(clust2==clusto1) # Efficiency is 0.5393
